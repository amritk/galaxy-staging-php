...
namespace DemoAPIScalarGalaxy\Core;
...
final class Util
{
    ...
    public const JSON_ENCODE_FLAGS =
    ...
    public const STREAMING_CONTENT_TYPE = [
        '/^text\/event-stream/',
    ...
    public static function strVal(mixed $value): string
    {
        ...
        if (
            is_object($value) && is_a($value, class: \DateTimeInterface::class)
        ) {
    ...
    {
        $mapped = match (true) {
            is_array($value) => array_map(static fn($v) => self::mapRecursive(
                $callback,
                value: $v,
            ), $value),
            default => $value,
    ...
    {
        $mapped = self::mapRecursive(static fn($vs) => is_array($vs)
            && !array_is_list($vs)
                ? array_filter($vs, callback: static fn($v) => !is_null($v))
                : $vs, value: $value);
    ...
    public static function parsePath(string|array $path): string
    {
        ...
        [$template] = $path;
        $mapped = array_map(
            static fn($s) => rawurlencode(self::strVal($s)),
            array: array_slice($path, 1),
        );
    ...
    public static function joinUri(
        ...
        if ($path = $parsed['path'] ?? null) {
            $base = str_starts_with($path, '/')
                ? $base->withPath($path)
                : $base->withPath($base->getPath() . '/' . $path);
        }
        ...
        $normalizedQuery = self::mapRecursive(static fn($v) => is_bool($v)
            || is_numeric($v)
                ? self::strVal($v)
                : $v, value: $mergedQuery);
        $qs = http_build_query(
    ...
    public static function isStreamingRequest(RequestInterface $request): bool
    {
        ...
        return !empty(array_filter(
            ...
            static fn(string $pattern) => (bool) preg_match(
                $pattern,
                subject: $accept,
            ),
        ));
    ...
    public static function withSetBody(
        ...
        if (preg_match('/^multipart\/form-data/', $contentType)) {
            ...
            $encoded = implode('', iterator_to_array(
                $gen,
                preserve_keys: false,
            ));
            $stream = $factory->createStream($encoded);
    ...
    {
        return (
            json_encode($obj, flags: JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT)
            ?: ''
        );
    }
    ...
    private static function writeMultipartChunk(
        ...
        yield "\r\n";
        foreach (self::writeMultipartContent(
            $val,
            closing: $closing,
        ) as $chunk) {
            yield $chunk;
    ...
    private static function writeMultipartField(
        ...
        if (is_array($val) && array_is_list($val)) {
            foreach ($val as $item) {
                yield from self::writeMultipartField(
                    boundary: $boundary,
                    key: $key,
                    val: $item,
                    closing: $closing,
            ...
            yield from self::writeMultipartChunk(
                boundary: $boundary,
                key: $key,
                val: $val,
                closing: $closing,
    ...
    {
        $boundary = rtrim(
            strtr(base64_encode(random_bytes(60)), '+/', '-_'),
            '=',
        );
            ...
            try {
                if (is_array($body) || is_object($body)) {
                    foreach ((array) $body as $key => $val) {
                        yield from static::writeMultipartField(
                            boundary: $boundary,
                            key: $key,
                            val: $val,
                            closing: $closing,
