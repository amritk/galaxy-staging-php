...
namespace DemoAPIScalarGalaxy\Core\Concerns;
...
use DemoAPIScalarGalaxy\Core\Conversion\ModelOf;
...
trait SdkModel
{
    ...
    public function __serialize(): array
    {
        ...
        return array_map(static fn($v) => self::serialize(
            $v,
        ), array: $properties);
    }
    ...
    public function __get(string $key): mixed
    {
        if (!array_key_exists($key, array: self::$converter->properties)) {
            throw new \RuntimeException(
                "Property '{$key}' does not exist in {$this}::class",
            );
        }
    ...
    public function offsetSet(mixed $offset, mixed $value): void
    {
        ...
        $coerced = Conversion::coerce(
            $type,
            value: $value,
            state: new CoerceState(translateNames: false),
        );
