...
use DemoAPIScalarGalaxy\Core\Contracts\BasePage;
use DemoAPIScalarGalaxy\Core\Contracts\BaseResponse;
use DemoAPIScalarGalaxy\Core\Contracts\BaseStream;
...
use DemoAPIScalarGalaxy\Core\Exceptions\APIConnectionException;
use DemoAPIScalarGalaxy\Core\Exceptions\APIStatusException;
use DemoAPIScalarGalaxy\Core\Implementation\RawResponse;
...
abstract class BaseClient
{
    ...
    protected function buildRequest(
        ...
        $idempotencyHeaders = $this->idempotencyHeader
        && !array_key_exists($this->idempotencyHeader, array: $headers)
    ...
    protected function followRedirect(
        ...
        if (!$location) {
            throw new APIConnectionException(
                $req,
                message: 'Redirection without Location header',
            );
        }
    ...
    protected function sendRequest(
        ...
        $req = $req->withHeader('X-Scalar-Retry-Count', strval($retryCount));
        ...
        if ($code >= 400 || is_null($rsp)) {
            ...
            time_nanosleep(
                (int) $floor,
                nanoseconds: (int) ($seconds - $floor) * (10 ** 9),
            );
