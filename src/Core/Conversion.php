...
use DemoAPIScalarGalaxy\Core\Conversion\EnumOf;
...
final class Conversion
{
    ...
    {
        if (is_array($value)) {
            return array_map(static fn($v) => self::dump_unknown(
                $v,
                state: $state,
            ), array: $value);
        }
        ...
        if (is_object($value)) {
            ...
            return empty($acc)
                ? (object) $acc
                : self::dump_unknown($acc, state: $state);
        }
    ...
    public static function coerce(
        Converter|ConverterSource|string $target,
        mixed $value,
        CoerceState $state = new CoerceState(),
    ): mixed {
        ...
        if (is_a($target, class: \BackedEnum::class, allow_string: true)) {
            return EnumOf::fromBackedEnum($target)->coerce(
                $value,
                state: $state,
            );
        }
