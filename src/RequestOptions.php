...
use DemoAPIScalarGalaxy\Core\Attributes\Required as Property;
...
final class RequestOptions implements BaseModel
{
    ...
    {
        $parsed = array_map(static fn($o) => $o instanceof self
            ? $o->toProperties()
            : $o ?? [], array: $options);
    ...
    public static function with(
        ...
        null !== $initialRetryDelay && ($self->initialRetryDelay =
            $initialRetryDelay);
        null !== $maxRetryDelay && ($self->maxRetryDelay = $maxRetryDelay);
        null !== $extraHeaders && ($self->extraHeaders = $extraHeaders);
        null !== $extraQueryParams && ($self->extraQueryParams =
            $extraQueryParams);
        null !== $extraBodyParams && ($self->extraBodyParams =
            $extraBodyParams);
        null !== $transporter && ($self->transporter = $transporter);
        null !== $streamingTransporter && ($self->streamingTransporter =
    ...
    public function withStreamingTransporter(ClientInterface $streamingTransporter): self
    {
        $self = clone $this;
